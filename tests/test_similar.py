...
def open_store(tmp_path: Path) -> GenericStore:
...
async def seed(
    ...
    store.store.conn.execute(INSERT_EVENT, (key, text))
...
def counts(store: GenericStore) -> tuple[int, int]:
    vec = store.store.conn.execute("SELECT COUNT(*) AS n FROM verdict_vectors").fetchone()["n"]
    evi = store.store.conn.execute("SELECT COUNT(*) AS n FROM verdict_evidence").fetchone()["n"]
    return vec, evi


def table_exists(store: GenericStore, name: str) -> bool:
    return store.store.conn.execute("SELECT 1 FROM sqlite_master WHERE name = ?", (name,)).fetchone() is not None
...
def test_record_verdict_inserts_one_vector_and_rerecord_upserts(tmp_path: Path, fake_embedder: None) -> None:
    async def go() -> dict[str, object]:
        with open_store(tmp_path) as store:
            store.store.conn.execute(INSERT_EVENT, ("k1", "always use uv not pip"))
            await store.record_verdict(
                DedupKey("k1"),
            ...
            return {"after_first": after_first, "after_upgrade": counts(store)}
...
def test_verdict_without_canonical_key_never_touches_the_vector_store(tmp_path: Path, fake_embedder: None) -> None:
    async def go() -> bool:
        with open_store(tmp_path) as store:
            store.store.conn.execute(INSERT_EVENT, ("k1", "some feedback"))
            await store.record_verdict(
                DedupKey("k1"),
            ...
            return table_exists(store, "verdict_vectors")
...
def test_full_to_full_noop_does_not_re_upsert_the_vector(tmp_path: Path, fake_embedder: None) -> None:
    async def go() -> list[tuple[object, object]]:
        with open_store(tmp_path) as store:
            store.store.conn.execute(INSERT_EVENT, ("k1", "use uv not pip"))
            await store.record_verdict(
                DedupKey("k1"),
            ...
            cur = store.store.conn.execute("SELECT canonical_key FROM verdict_evidence")
            evidence = [(row["canonical_key"],) for row in cur]
            return evidence

    assert asyncio.run(go()) == [("first-rule",)]
...
def test_dropping_canonical_key_on_upgrade_clears_stale_evidence(tmp_path: Path, fake_embedder: None) -> None:
    async def go() -> dict[str, object]:
        with open_store(tmp_path) as store:
            ...
            verdict_ck = store.store.conn.execute(
                "SELECT canonical_key FROM verdicts WHERE dedup_key = 'k1'"
...
def test_dropping_canonical_key_clears_evidence_on_a_fresh_connection(tmp_path: Path, fake_embedder: None) -> None:
    async def go() -> dict[str, object]:
        ...
        with open_store(tmp_path) as store:
            ...
            return {"seeded": seeded, "cleared": counts(store)}
...
def test_judge_and_auditor_evidence_coexist_on_one_event(tmp_path: Path, fake_embedder: None) -> None:
    async def go() -> tuple[tuple[int, int], list[tuple[object, object]]]:
        with open_store(tmp_path) as store:
            ...
            cur = store.store.conn.execute("SELECT role, canonical_key FROM verdict_evidence ORDER BY role")
            rows = [(row["role"], row["canonical_key"]) for row in cur]
            return counts(store), rows

    (vec, evi), rows = asyncio.run(go())
    assert (vec, evi) == (2, 2)
...
def test_concurrent_record_verdict_prepares_the_fresh_connection_once(tmp_path: Path, fake_embedder: None) -> None:
    async def go() -> tuple[tuple[int, int], int]:
        with open_store(tmp_path) as store:
            for i in range(4):
                store.store.conn.execute(INSERT_EVENT, (f"k{i}", f"use uv variant {i}"))
            ...
            async with asyncio.TaskGroup() as tg:
                for i in range(4):
                    tg.create_task(record(i))
            verdicts = store.store.conn.execute("SELECT COUNT(*) AS n FROM verdicts").fetchone()["n"]
            return counts(store), verdicts

    (vec, evi), verdicts = asyncio.run(go())
    assert (vec, evi) == (4, 4)
    assert verdicts == 4
...
def test_record_verdict_with_canonical_key_fails_loud_and_writes_no_verdict_without_the_extra(
    ...
    async def go() -> int:
        with open_store(tmp_path) as store:
            ...
            return store.store.conn.execute("SELECT COUNT(*) AS n FROM verdicts").fetchone()["n"]
