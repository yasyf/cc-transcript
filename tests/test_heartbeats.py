...
def test_beat_creates_one_row_with_count_one(tmp_path: Path) -> None:
    log = HeartbeatLog.open(tmp_path / "decisions.db")
    log.beat("sess-a", "PreToolUse", 1000)
    assert log.for_session("sess-a") == (
        Heartbeat(session_id="sess-a", event="PreToolUse", first_ts_ms=1000, last_ts_ms=1000, count=1),
    )


def test_beat_upserts_count_and_last_ts_keeping_first(tmp_path: Path) -> None:
    log = HeartbeatLog.open(tmp_path / "decisions.db")
    log.beat("sess-a", "PreToolUse", 1000)
    log.beat("sess-a", "PreToolUse", 1500)
    log.beat("sess-a", "PreToolUse", 2000)
    (beat,) = log.for_session("sess-a")
    assert beat == Heartbeat(
        session_id="sess-a", event="PreToolUse", first_ts_ms=1000, last_ts_ms=2000, count=3
    )


def test_distinct_events_are_distinct_rows(tmp_path: Path) -> None:
    log = HeartbeatLog.open(tmp_path / "decisions.db")
...
def test_for_session_orders_by_first_dispatch(tmp_path: Path) -> None:
    log = HeartbeatLog.open(tmp_path / "decisions.db")
    log.beat("sess-a", "Stop", 300)
    log.beat("sess-a", "PreToolUse", 100)
    log.beat("sess-a", "Stop", 400)  # later, but Stop's first was 300
    log.beat("sess-a", "UserPromptSubmit", 200)
    assert [b.event for b in log.for_session("sess-a")] == ["PreToolUse", "UserPromptSubmit", "Stop"]


def test_reopen_persists_and_keeps_accumulating(tmp_path: Path) -> None:
    db = tmp_path / "decisions.db"
    HeartbeatLog.open(db).beat("sess-a", "PreToolUse", 100)
    HeartbeatLog.open(db).beat("sess-a", "PreToolUse", 250)
    (beat,) = HeartbeatLog.open(db).for_session("sess-a")
    assert beat.count == 2
    assert beat.first_ts_ms == 100
    assert beat.last_ts_ms == 250


def test_coexists_with_decisions_table_in_same_file(tmp_path: Path) -> None:
    ...
    db = tmp_path / "decisions.db"
    hb = HeartbeatLog.open(db)
    hb.beat("sess-a", "PreToolUse", 100)
    dec = DecisionLog.open(db)
    dec.append(
        Decision(
            ts_ms=100,
            session_id="sess-a",
