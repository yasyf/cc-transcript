...
Rust ``_native.notifications_replay`` reproduces each replay exactly and that the
...
def test_rust_notifications_replay_matches_golden(entry: dict) -> None:
    assert _native.notifications_replay(to_bytes(tuple(entry["records"]))) == entry["expected"]
...
def test_dup_keys_replay_last_wins(raw: str) -> None:
    ...
    assert _native.notifications_replay(raw.encode()) == expected
