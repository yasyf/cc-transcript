"""Golden pin: the native events-in bucketing reproduces the frozen bucketing.
...
synthetic transcripts. This asserts the events-in
``_native.bucket_events_from_events`` binding behind
``ConversationBucketer.bucket_events`` reproduces each bucketing exactly, and that
``extract_bucket_keys`` stays consistent with it.
"""
...
def test_rust_bucket_events_from_events_matches_golden(entry: dict) -> None:
    events = testkit.parse_events(*entry["records"])
    buckets = [
        {
            "session_id": bucket["session_id"],
            "bucket_index": bucket["bucket_index"],
            "bucket_start_ms": bucket["bucket_start_ms"],
            "uuids": [events[index].meta.uuid for index in bucket["indices"]],
        }
        for bucket in _native.bucket_events_from_events(events)
    ]
    assert buckets == entry["buckets"]
