...
The Python ``cc_transcript.ids`` reference and the Rust ``_parser_rs`` pyfunctions
...
def test_corpus_canonical_and_digest_parity(row: dict[str, Any]) -> None:
    ...
    assert _parser_rs.ids_canonical_json(value_json) == row["canonical"]
    assert tool_digest(row["tool"], row["input"]) == row["digest"]
    assert _parser_rs.ids_tool_digest(row["tool"], value_json) == row["digest"]
...
def test_tricky_canonical_parity(row: dict[str, Any]) -> None:
    ...
    assert _parser_rs.ids_canonical_json(json.dumps(row["value"])) == row["canonical"]
...
def test_raw_json_canonical_parity(row: dict[str, Any]) -> None:
    ...
    assert _parser_rs.ids_canonical_json(row["json"]) == row["canonical"]
...
def test_out_of_range_integers_reject_on_both_sides(row: dict[str, Any]) -> None:
    ...
    with pytest.raises(ValueError):
        _parser_rs.ids_canonical_json(row["json"])
...
def test_random_float_shortest_repr_fuzz() -> None:
    ...
    for value in values:
        ...
        assert _parser_rs.ids_canonical_json(payload) == canonical_json(value).decode(), payload
